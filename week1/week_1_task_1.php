...
/*
    ...
    Нужно разобраться $data с описанием статьей и выввести статьи у которых есть тег 
    Automotive Industry в виде html документа следующего формата:
        ...
        - перед выводом каждого "Body" в нем должна быть проведина замена из "ChangeMap" справа на лево, к примеру Volkswagen: 
        VW, значит все слова Volkswagen нужно заменить на VW
        - тег tags должен состоять из уникальных тегов из всех секций Tags разделенные ", ", 
        даже если тег встречаеться больше одного раза то все равно в tags должен быть только раз
        
    ПОЖЕЛАНИЯ:
        - всему даем адекватные имена, классам, методам, переменныем. 
        Чтоб было понятно что они делают, односимвольные переменные могут быть только для счетчика
        - используем классы, весь код которые делает чтото больше раза заворачиваем в методы, и даем понятное имя
        - отделяем логику от представления и используем массимы. 
        То есть лучше сначало разобрать все данные и разместить их в массив/массивы и потом
        в цикле выводить на экран чем зем за один проход сделать все
        - не забываем про коментарии
        - функции лучше делать мальенькие, до 20 строк, это не обязательное правило,
            но к этому нужно стермиться, в последствии вы поймете почему 20 и почему иногда нужно сделать больше
